...
<title>Tjekind PHP Cloud: BFJ</title>
...
<div class="title">BFJ </div>  </div>
</div><!--header-->
<div class="contents">
<div class="textblock"><p><a href="https://gitlab.com/philbooth/bfj/pipelines"><object type="image/svg+xml" data="https://gitlab.com/philbooth/bfj/badges/master/pipeline.svg" style="pointer-events: none;">Build status</object></a> <a href="https://www.npmjs.com/package/bfj"><object type="image/svg+xml" data="https://img.shields.io/npm/v/bfj.svg" style="pointer-events: none;">Package status</object></a> <a href="https://www.npmjs.com/package/bfj"><object type="image/svg+xml" data="https://img.shields.io/npm/dm/bfj.svg" style="pointer-events: none;">Downloads</object></a> <a href="https://opensource.org/licenses/MIT"><object type="image/svg+xml" data="https://img.shields.io/npm/l/bfj.svg" style="pointer-events: none;">License</object></a></p>
...
<li><a href="#what-other-methods-are-there">What other methods are there?</a><ul>
...
<li><a href="#what-options-can-i-specify">What options can I specify?</a><ul>
...
<h1><a class="anchor" id="autotoc_md5403"></a>
...
<h1><a class="anchor" id="autotoc_md5404"></a>
Is it fast?</h1>
<p>No.</p>
<p>BFJ yields frequently to avoid monopolising the event loop, interrupting its own execution to let other event handlers run. The frequency of those yields can be controlled with the <a href="#what-options-can-i-specify"><code>yieldRate</code> option</a>, but fundamentally it is not designed for speed.</p>
<p>Furthermore, when serialising data to a stream, BFJ uses a fixed-length buffer to avoid exhausting available memory. Whenever that buffer is full, serialisation is paused until the receiving stream processes some more data, regardless of the value of <code>yieldRate</code>. You can control the size of the buffer using the <a href="#options-for-serialisation-functions"><code>bufferLength</code> option</a> but really, if you need quick results, BFJ is not for you.</p>
<h1><a class="anchor" id="autotoc_md5405"></a>
...
<ul>
<li><a href="#how-do-i-read-a-json-file"><code>read</code></a> asynchronously parses a JSON file from disk.</li>
<li><a href="#how-do-i-parse-a-stream-of-json"><code>parse</code> and <code>unpipe</code></a> are for asynchronously parsing streams of JSON.</li>
<li><a href="#how-do-i-selectively-parse-individual-items-from-a-json-stream"><code>match</code></a> selectively parses individual items from a JSON stream.</li>
<li><a href="#bfjwalk-stream-options"><code>walk</code></a> asynchronously walks a stream, emitting events as it encounters JSON tokens. Analagous to a <a href="http://en.wikipedia.org/wiki/Simple_API_for_XML">SAX parser</a>.</li>
</ul>
<p>The other four functions handle the reverse transformations, serialising JavaScript data to JSON:</p>
<ul>
<li><a href="#how-do-i-write-a-json-file"><code>write</code></a> asynchronously serialises data to a JSON file on disk.</li>
<li><a href="#how-do-i-create-a-stream-of-json"><code>streamify</code></a> asynchronously serialises data to a stream of JSON.</li>
<li><a href="#how-do-i-create-a-json-string"><code>stringify</code></a> asynchronously serialises data to a JSON string.</li>
<li><a href="#bfjeventify-data-options"><code>eventify</code></a> asynchronously traverses a data structure depth-first, emitting events as it encounters items. By default it coerces promises, buffers and iterables to JSON-friendly values.</li>
</ul>
<h1><a class="anchor" id="autotoc_md5406"></a>
...
</div><!-- fragment --><h1><a class="anchor" id="autotoc_md5407"></a>
...
<p>It takes two arguments; the path to the JSON file and an <a href="#options-for-parsing-functions">options</a> object.</p>
...
<h1><a class="anchor" id="autotoc_md5408"></a>
...
<p class="startli">It takes two arguments; a <a href="https://nodejs.org/api/stream.html#stream_readable_streams">readable stream</a> from which the JSON will be parsed and an <a href="#options-for-parsing-functions">options</a> object.</p>
...
<p class="startli">It takes two arguments; a callback function that will be called after parsing is complete and an <a href="#options-for-parsing-functions">options</a> object.</p>
...
<h1><a class="anchor" id="autotoc_md5409"></a>
How do I selectively parse individual items from a JSON stream?</h1>
<div class="fragment"><div class="line">const bfj = require(&#39;bfj&#39;);</div>
<div class="line"> </div>
<div class="line">// Call match with your stream and a selector predicate/regex/string</div>
...
<h1><a class="anchor" id="autotoc_md5410"></a>
...
<p>It takes three arguments; the path to the JSON file, the data structure to serialise and an <a href="#options-for-serialisation-functions">options</a> object.</p>
<h1><a class="anchor" id="autotoc_md5411"></a>
...
<p>It takes two arguments; the data structure to serialise and an <a href="#options-for-serialisation-functions">options</a> object.</p>
<p>If there a circular reference is encountered in the data and <code>options.circular</code> is not set to &lsquo;'ignore&rsquo;<code>, a</code>dataError` event will be emitted. If any other errors occur, an <code>error</code> event will be emitted.</p>
<h1><a class="anchor" id="autotoc_md5412"></a>
...
<p>It takes two arguments; the data structure to serialise and an <a href="#options-for-serialisation-functions">options</a> object.</p>
<h1><a class="anchor" id="autotoc_md5413"></a>
What other methods are there?</h1>
<h2><a class="anchor" id="autotoc_md5414"></a>
...
<p>It takes two arguments; a <a href="https://nodejs.org/api/stream.html#stream_readable_streams">readable stream</a> from which the JSON will be read and an <a href="#options-for-parsing-functions">options</a> object.</p>
...
<li><code>bfj.events.endLine</code> indicates that a root-level newline character has been encountered in an <a href="#can-it-handle-newline-delimited-json-ndjson">NDJSON</a> stream. Only emitted if the <code>ndjson</code> <a href="#options-for-parsing-functions">option</a> is set.</li>
</ul>
...
<h2><a class="anchor" id="autotoc_md5415"></a>
...
<p>It takes two arguments; the data structure to traverse and an <a href="#options-for-serialisation-functions">options</a> object.</p>
<p>The emitted events are defined as public properties of an object, <code>bfj.events</code>:</p>
<ul>
...
<li><code>bfj.events.literal</code> indicates that a JSON literal (either <code>true</code>, <code>false</code> or <code>null</code>) has been encountered. The listener will be passed the value as its argument.</li>
<li><code>bfj.events.error</code> indicates that an error was caught from one of the event handlers in user code. The listener will be passed the <code>Error</code> instance as its argument.</li>
<li><code>bfj.events.dataError</code> indicates that a circular reference was encountered in the data and the <code>circular</code> option was not set to &lsquo;'ignore&rsquo;`. The listener will be passed an <code>Error</code> instance as its argument.</li>
<li><code>bfj.events.end</code> indicates that the end of the data has been reached and no further events will be emitted.</li>
</ul>
<h1><a class="anchor" id="autotoc_md5416"></a>
What options can I specify?</h1>
<h2><a class="anchor" id="autotoc_md5417"></a>
...
<li><code>options.ndjson</code>: If set to <code>true</code>, newline characters at the root level will be treated as delimiters between discrete chunks of JSON. See <a href="#can-it-handle-newline-delimited-json-ndjson">NDJSON</a> for more information.</li>
...
<h2><a class="anchor" id="autotoc_md5418"></a>
...
<h1><a class="anchor" id="autotoc_md5419"></a>
Is it possible to pause parsing or serialisation from calling code?</h1>
<p>Yes it is! Both <a href="#bfjwalk-stream-options"><code>walk</code></a> and <a href="#bfjeventify-data-options"><code>eventify</code></a> decorate their returned event emitters with a <code>pause</code> method that will prevent any further events being emitted. The <code>pause</code> method itself returns a <code>resume</code> function that you can call to indicate that processing should continue.</p>
<p>For example:</p>
<div class="fragment"><div class="line">const bfj = require(&#39;bfj&#39;);</div>
<div class="line">const emitter = bfj.walk(fs.createReadStream(path), options);</div>
<div class="line"> </div>
...
</div><!-- fragment --><h1><a class="anchor" id="autotoc_md5420"></a>
Can it handle &lt;a href="http://ndjson.org/"&gt;newline-delimited JSON (NDJSON)&lt;/a&gt;?</h1>
<p>Yes. If you pass the <code>ndjson</code> <a href="#options-for-parsing-functions">option</a> to <code>bfj.walk</code>, <code>bfj.match</code> or <code>bfj.parse</code>, newline characters at the root level will act as delimiters between discrete JSON values:</p>
<ul>
...
<h1><a class="anchor" id="autotoc_md5421"></a>
...
<h1><a class="anchor" id="autotoc_md5422"></a>
...
<h1><a class="anchor" id="autotoc_md5423"></a>
Is there a change log?</h1>
<p><a class="el" href="vary_2_history_8md.html">Yes</a>.</p>
<h1><a class="anchor" id="autotoc_md5424"></a>
...
<h1><a class="anchor" id="autotoc_md5425"></a>
...
<h1><a class="anchor" id="autotoc_md5426"></a>
